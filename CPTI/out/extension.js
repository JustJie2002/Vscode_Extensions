...
function activate(context) {
    ...
    let foo = vscode.commands.registerCommand('CPTI.show_path', () => {
        ...
        if (tpath == null || !fs_1.default.existsSync(tpath)) {
            vscode.window.showInformationMessage("Please go to the setting to set the absolute to your CP templates.");
            return;
        }
        var files = fs_1.default.readdirSync(tpath, { withFileTypes: true })
        ...
        vscode.window.showQuickPick(files).then(selectedString => {
            if (selectedString) {
                const fileContent = fs_1.default.readFileSync(tpath + selectedString, 'utf-8');
